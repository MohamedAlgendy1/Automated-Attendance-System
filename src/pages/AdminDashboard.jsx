...
function AdminDashboard() {
  ...
  // ================= STUDENTS (FIXED: from backend ONLY) =================
  const [students, setStudents] = useState([]);
  const [studentsLoading, setStudentsLoading] = useState(false);
  const [studentSearch, setStudentSearch] = useState("");
  ...
  // ================= STUDENTS (FIXED SAFE ENDPOINT CHECK) =================
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      ...
      try {
        ...
        // endpoint may not exist → safe fallback
        setStudents([]);
        console.log("Students endpoint missing or error:", getErrorMessage(err));
      } finally {
        setStudentsLoading(false);
      }
    };
    ...
    fetchStudents();
  }, [activePage, studentSearch]);

  // ================= DELETE STUDENT (LOCAL ONLY SAFE) =================
  const deleteStudent = (id) => {
      ...
      <div className="main">
        ...
        {activePage === "reports" && (
          <div className="table-box">
            <h2>Reports</h2>
            <p>
              Lecturers: {stats?.lecturersCount || 0} | Students: {stats?.studentsCount || 0}
            </p>
          </div>
        )}
      </div>
