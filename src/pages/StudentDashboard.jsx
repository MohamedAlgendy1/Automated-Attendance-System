...
function StudentDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // ✅ إزالة التكرار — نفس courseLectureId يحسب مرة واحدة بس
        const uniqueAttendance = Object.values(
          rawAttendance.reduce((acc, a) => {
        ...
        const lecturesCount = lecturesArrays.reduce((sum, arr) => sum + arr.length, 0);
        setTotalLectures(lecturesCount);
  ...
  // ✅ حساب صح بعد الـ deduplication
  const attendedLectures = attendance.filter((a) => a.status === "Present").length;
            ...
            {loading ? (
              ...
              <div className="courses-grid">
                {myCourses.map((course) => {
                  ...
                  // ✅ حساب الحضور لكل كورس بعد الـ deduplication
                  const attended = lectures.filter((lec) =>
                    attendance.some(
                      (a) => a.courseLectureId === lec.id && a.status === "Present"
                    )
                  ).length;
